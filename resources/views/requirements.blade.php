...
@section('header')
    <h2 class="text-3xl font-semibold tracking-tight text-gray-900">Server Requirements</h2>
...
@section('content')
    ...
    @if(count(array_unique($requirements)) === 1)
        <div class="mt-5">
            <a href="{{ route('app-installer::configuration.index') }}" class="flex w-full justify-center rounded-md border border-transparent bg-slate-900 py-3 px-4 text-sm font-medium text-white shadow-sm hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-700 focus:ring-offset-2">Next Step</a>
        </div>
    @else
